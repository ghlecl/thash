...
/* External Interface */
   SHA256(); // constructor.  Sets up initial state_.
   ...
   ~SHA256(); // destructor.  Clears out state_ and buffered data.

/* Internal Interface (public, for debug's sake) */
   // This is the internal state_ of the hash.
   UInt32 state_[8];

   // This is how many message bytes we've seen so far.
   size_t count_;

   // This buffer_s up to a whole block of data
   Byte buffer_[64];
   ...
   // Process the finished block of data in "buffer_"
   void block();
};
...
UInt32 ror(UInt32 value,UInt32 bitcount_)
*/
...
/* This sets up the Sha256 initial state_, at the start of a message. */
void SHA256::init()
{
  state_[0] = 0x6a09e667;
  state_[1] = 0xbb67ae85;
...
/* This adds another block of data to our current state_.
   This is our main transforming/mixing function. */
// equivalent to processChunk
void SHA256::block()
{
   unsigned i;
   ...
   UInt32 W[16]; // Work buffer_: 0-15 are straight from the data
   for( i = 0; i < 16; ++i )
   {
      W[i]= ( (UInt32)( buffer_[i * 4    ] ) << 24 ) +
            ( (UInt32)( buffer_[i * 4 + 1] ) << 16 ) +
            ( (UInt32)( buffer_[i * 4 + 2] ) <<  8 ) +
            ( (UInt32)( buffer_[i * 4 + 3] )           ); // big-endian 32-bit load

      KWi[i] = W[i] + K[i];
   }

   // The rest of W is a scrambled copy of the original data
   for( ; i<ROUND_count_; ++i )
   {
      W[i&15] += s1( W[(i-2)&15] ) + W[(i-7)&15] + s0( W[(i-15)&15] );
      KWi[i] = W[i&15] + K[i];
   }

   UInt32 a,b,c,d,e,f,g,h; /* local copies of state_, for performance */
   ...
   // Add result back into state_ array
...
void SHA256::add(const void *data, size_t size)
{
   ...
   UInt32 curBufferPos( (UInt32)count_ & 0x3F ); /* location within last block */
   while ( size > 0 )
   {
      buffer_[curBufferPos++] = *dataptr++; // copy next byte of data
      count_++; // message got longer
...
{
   size_t lenInBits = (count_ << 3); // i.e., times 8 bits per byte
   UInt32 curBufferPos = (UInt32)count_ & 0x3F; // 0x3f is mask to wrap around to buffer size
   unsigned i;
   buffer_[curBufferPos++] = 0x80; // standard specifies "add a one bit...
   while (curBufferPos != (64 - 8)) // ...then pad with zeros to end of block"
   {
      ...
      buffer_[curBufferPos++] = 0; // zero out rest of block
   }

   // Finally, add message length, in bits, as big-endian 64 bit number
   for (i = 0; i < 8; i++)
   {
      buffer_[curBufferPos++] = (Byte)(lenInBits >> 56);
      lenInBits <<= 8;
   }

   block(); // transform last block (including length)

   // Copy state_ out as big-endian integers.
   SHA256::digest output;
   for (i = 0; i < 8; i++)
   {
      output.data[i*4+0] = (Byte)(state_[i] >> 24);
      output.data[i*4+1] = (Byte)(state_[i] >> 16);
      output.data[i*4+2] = (Byte)(state_[i] >> 8);
      output.data[i*4+3] = (Byte)(state_[i]);
   }
...
SHA256::~SHA256()
{
   // To keep from leaving any sensitive data lying around, zero out our buffer_s.
